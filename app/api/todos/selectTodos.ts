...
async function selectTodos(connection: Connection): Promise<TodoItemData[]> {
	...
	const todoIDs = (todoRows as Partial<TodoItemData>[]).map(
		(result) => result.id || 'NULL'
	);
	const notesPromise = connection.query(
		'SELECT * FROM notes WHERE todo_id IN (?)',
		[todoIDs]
	);

	const todos = todoRows.map(
		(result: Partial<TodoItemData>): TodoItemData => ({
			id: result.id || '',
			title: result.title || '',
