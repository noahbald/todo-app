...
async function insertTodos(connection: Connection, todos: TodoItemData[]) {
	...
	promises.push(
		connection
			.query(
				'INSERT INTO todos (id, title, description, priority, deadline, status) VALUES ?',
				[todoData]
			)
			.then(([result]) =>
				console.log('Inserted', (result as any).affectedRows, 'todos')
			)
	);

	const notesData = todos.reduce<string[][]>((notes, todo) => {
		return [...notes, ...todo.notes.map((note) => [todo.id, note])];
	}, []);
	if (notesData.length) {
		promises.push(
			connection
				.query('INSERT INTO notes (todo_id, note) VALUES ?', [
					notesData,
				])
				.then(([result]) =>
					console.log(
